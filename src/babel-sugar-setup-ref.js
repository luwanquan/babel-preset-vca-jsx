...
const isRefCallee = (t, path) => {
  ...
  return (
    init.node.callee.name === "ref" && t.isNullLiteral(init.get("arguments")[0])
  );
};
...
const autoImportLifeCycle = (t, path, lifeCycles = []) => {
  ...
  if (!imported) {
    path.unshiftContainer(
      "body",
      t.importDeclaration(
        genImportSpecifiers(lifeCycles),
        t.stringLiteral(importSource)
      )
    );
    return lifeCycles;
...
const injectLifeCycle = (t, path, lifeCycles = [], refs = []) => {
  ...
  if (!ctx) {
    ...
    if (t.isObjectPattern(ctx)) {
      const refsProp = ctx.node.properties.find(
        node => node.key.name === props[0]
      );
      if (!refsProp) {
        ...
        t.logicalExpression(
          ...
          t.memberExpression(
            identifiers.length > 1
              ? t.memberExpression(...identifiers)
              : identifiers[0],
            t.identifier(val)
          ),
          t.nullLiteral()
  ...
  const nextTick = t.expressionStatement(
    t.callExpression(
      t.memberExpression(t.identifier("Vue"), t.identifier("nextTick")),
    ...
    t.expressionStatement(
      t.callExpression(t.identifier(val), [
        t.arrowFunctionExpression(
          [],
          t.blockStatement([...statements, nextTick])
        )
      ])
    )
  );
      ...
      Program(p) {
        p.traverse({
          "ObjectMethod|ObjectProperty"(path) {
            ...
            autoImportVue(t, p);
            const lifeCycles = autoImportLifeCycle(t, p, [
              "onMounted",
              "onUpdated"
            ]);
